...
![Extension demo](../../../../assets/extensions/extension-demo.png)
...
![Message flow](../../../../assets/extensions/message-flow.png)
*Message flow*

**1. Define extension class and the `on_input` listener**

Create a subclass of `Extension`.
...
Return a list of `Result` in order to render results.
...
`on_enter` an action that will run when the result is activated with enter keypress (or by clicking).
...
Often built-in [actions](./actions) are not enough and you need to define your own action on item enter. Here's how to do it.

**1. Pass custom data with ExtensionCustomAction**

Instantiate `Result` with `on_enter` that is instance of `ExtensionCustomAction`
...
![Custom action result](../../../../assets/extensions/custom-action-result.png)
...
Refer to this [requirements.txt documentation](https://pip.pypa.io/en/stable/reference/requirements-file-format/) for more information.
